...
mod = SourceModule("""
  __global__ void step(int *C, int *M)
  {
    ...
    if(k==0) {k_down=n_x-1;} else {k_down=k-1;}
    if(k==n_x-1) {k_up=0;} else {k_up=k+1;}

    count = C[k*n_x*n_x+j*n_x+i_left] + C[k*n_x*n_x+j_down*n_x+i] + C[k*n_x*n_x+j*n_x+i_right] + C[k*n_x*n_x+j_up*n_x+i]
          + C[k_down*n_x*n_x+j*n_x+i] + C[k_up*n_x*n_x+j*n_x+i];


      //+ C[j_down*n_x+i_right] + C[j_down*n_x+i_left]
      //+ C[j_up*n_x+i_right] + C[j_up*n_x+i_left];
  ...
  //A cell "Survives" if it has 2 or 3 living neighbours; it dies otherwise.
    if(count < 2 || count > 5) M[threadId] = 0; // cell dies
    if(count > 2 && count < 5) M[threadId] = C[threadId];// cell stays the same
    if(count == 5) M[threadId] = 1; // cell either stays alive, or is born
  }
""")
